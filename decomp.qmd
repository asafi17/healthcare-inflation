...
$$
\pi_t^L = \Delta P_t^L = \underbrace{\sum_i w_{i,t-k} \times \Delta p_i}_{\text{Price Component}} + \underbrace{\sum_i \Delta w_i \times p_{i, t}}_{\text{Weight Component}} 
$$
...
$$
\pi_t^P = \Delta P_{t-k,t}^P = \underbrace{\sum_i w_{i,t} \times \Delta p_i}_{\text{Price Component}} + \underbrace{\sum_i \Delta w_i \times p_{i, t-k}}_{\text{Weight Component}} 
$$
...
We will read the expenditure share data from the link [CMS NHEA](https://www.cms.gov/files/zip/nhe-tables.zip). 
...
inner_join(exp_monthly_long, ppi_long, by = c('date', 'payerType')) |> 
  ...
  group_by(date) |> 
  mutate(
    check = sum(Contribution, na.rm = T)
  ) |> 
  ungroup() |> 
  left_join(inflation, by = 'date') |> 
  mutate(pi_t = pi_t*100) |> 
  ggplot(aes(x = date)) + 
  geom_col(aes(y = Contribution, color = Component, fill = Component), alpha = .7) + 
  geom_point(aes(y = pi_t)) + 
  geom_line(aes(y = pi_t), linetype = 'dashed') + 
  # geom_ribbon(aes(ymin = 0, ymax = Contribution, fill = Component), alpha = 0.3) +
