...
## Industry-Based Hospital Inflation
...
ppi_yearly = ppi |> 
  ...
  summarise(
    ...
    `ppi_Private/Other` = mean(`ppi_Private/Other`, na.rm = T)
  )|> 
  ungroup()

ppi_yearly |> 
  rename(`ppi_Private/Other` = `ppi_Private/Other`) |> 
  pivot_longer(
    cols = !year, 
    names_to = 'Series', 
    values_to = 'Value'
...
### Compute Overall Hospital Inflation (Monthly)
...
### Compute Overall Hospital Inflation (Annual)
...
ppi |> 
  ...
  geom_line() + 
  labs(
    y = 'Producer Price Index - Commodity Based',
    x = 'Date'
  ) + 
